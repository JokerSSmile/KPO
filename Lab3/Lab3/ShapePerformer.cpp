...
void ShapePerformer::ParseLine(const std::string& line)
{
	...
	std::vector<int> params(splittedString.size() - 1);
	std::transform(splittedString.begin() + 1, splittedString.end(), params.begin(), [](const std::string& value) { return std::stoi(value); });
